...
$item = ['number_id'=>'','number_text'=>'','number_name'=>'','sign_image'=>'','number_image'=>'','coloring_image'=>'','number_video'=>''];
$pageTitle = 'إضافة رقم إنجليزي جديد';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($number_id < 0) $error = 'يرجى إدخال رقم صحيح.';
        ...
        $oldNumberImage = trim($_POST['default_number_image'] ?? '') ?: defaultNumberImagePath($number_id);
        ...
        if ($error === '') {
            $subject='اللغة الإنجليزية'; $type='الأرقام الإنجليزية';
            $checkSql = "SELECT id FROM lessons WHERE subject_name=? AND lesson_type=? AND letter_id=? LIMIT 1";
            $checkStmt = mysqli_prepare($conn,$checkSql); mysqli_stmt_bind_param($checkStmt,'ssi',$subject,$type,$number_id); mysqli_stmt_execute($checkStmt); $oldRow=mysqli_fetch_assoc(mysqli_stmt_get_result($checkStmt));
            if ($oldRow) {
                $oldId=intval($oldRow['id']);
                $update="UPDATE lessons SET lesson_title=?, example_word=?, lesson_image=?, custom_sign=?, coloring_image=?, card_video=? WHERE id=?";
                $stmt=mysqli_prepare($conn,$update); mysqli_stmt_bind_param($stmt,'ssssssi',$number_text,$number_name,$number_image,$sign_image,$coloring_image,$number_video,$oldId);
            } else {
                $supCreator = $isSupervisor ? intval($_SESSION['supervisor_id'] ?? 0) : 0;
                $insert="INSERT INTO lessons (subject_name,lesson_type,letter_id,lesson_title,example_word,lesson_image,custom_sign,coloring_image,card_video,created_by_supervisor) VALUES (?,?,?,?,?,?,?,?,?,?)";
                $stmt=mysqli_prepare($conn,$insert); mysqli_stmt_bind_param($stmt,'ssissssssi',$subject,$type,$number_id,$number_text,$number_name,$number_image,$sign_image,$coloring_image,$number_video,$supCreator);
            }
...
foreach (['number_id'=>0] as $_k=>$_v) {}
if ($item['number_id'] === '') {
    $item['sign_image'] = defaultSignPath(0);
    $item['number_image'] = defaultNumberImagePath(0);
    $item['coloring_image'] = defaultColoringPath(0);
}
?>
<!DOCTYPE html>
