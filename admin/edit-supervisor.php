...
// ── ماستر الفواكه/الخضار ثم ماستر الحيوانات ──────────────
...
$curPerms = [];
$pr = mysqli_query($conn, "SELECT * FROM supervisor_permissions WHERE supervisor_id=$id");
if ($pr) while ($p = mysqli_fetch_assoc($pr)) $curPerms[$p['section']] = $p;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$full_name || !$username) {
        ...
        if ($password) {
            ...
            $us = mysqli_prepare($conn, "UPDATE supervisors SET full_name=?, username=?, password_hash=? WHERE id=?");
            mysqli_stmt_bind_param($us, 'sssi', $full_name, $username, $hash, $id);
        } else {
            $us = mysqli_prepare($conn, "UPDATE supervisors SET full_name=?, username=? WHERE id=?");
            mysqli_stmt_bind_param($us, 'ssi', $full_name, $username, $id);
        }
        ...
        foreach ($sections as $key => $s) {
            ...
            $ps = mysqli_prepare($conn, "INSERT INTO supervisor_permissions (supervisor_id,section,can_view,can_add,can_edit,can_delete) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE can_view=VALUES(can_view),can_add=VALUES(can_add),can_edit=VALUES(can_edit),can_delete=VALUES(can_delete)");
            mysqli_stmt_bind_param($ps, 'isiiii', $id, $key, $v, $a, $e, $d);
...
.form-layout{display:grid;grid-template-columns:1fr 1.8fr;gap:20px;align-items:start;}
...
.field input:focus{border-color:#2a84c9;box-shadow:0 0 0 3px rgba(42,132,201,.1);}
...
/* ── جدول الصلاحيات ── */
.perm-controls{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;}
...
.perm-table thead th{padding:10px 10px;font-size:10px;font-weight:900;color:#7a90a3;text-align:center;border-bottom:2px solid #f0f5fb;text-transform:uppercase;}
.perm-table thead th:first-child{text-align:right;}
.perm-table tbody tr{border-bottom:1px solid #f7fafd;transition:.15s;}
.perm-table tbody tr:hover{background:#f7fbff;}
.perm-table td{padding:8px 10px;}

/* ── فواصل المجموعات ── */
.group-sep td{background:#f0f5fb;padding:7px 14px;font-size:10px;font-weight:900;color:#7a90a3;letter-spacing:1px;text-transform:uppercase;border:none !important;}

/* ── مستويات التعشيش ── */
.indent-0{}
.indent-1 .sec-badge{margin-right:14px;}
.indent-2 .sec-badge{margin-right:28px;}

/* ── صف الماستر ── */
.row-master{background:#f0fff6 !important;}
.row-master:hover{background:#e4fded !important;}
/* ── زر ماستر ── */
...
.master-badge{display:inline-block;font-size:9px;font-weight:900;padding:2px 6px;border-radius:5px;margin-left:5px;vertical-align:middle;}
.master-gc{background:#28a75d;color:#fff;}
.master-sub{background:#00b4b4;color:#fff;}

.sec-badge{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;border-radius:9px;font-size:12px;font-weight:800;}
...
.toggle-slider::before{content:'';position:absolute;width:13px;height:13px;left:3px;top:3px;background:#fff;border-radius:50%;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.2);}
...
.toggle input:disabled+.toggle-slider{opacity:.35;cursor:not-allowed;}

.hint-box{font-size:11px;color:#28a75d;font-weight:800;padding:7px 10px;background:#e6ffec;border-radius:8px;border:1px solid #b8f0cc;margin-bottom:12px;}
...
.btn-submit{flex:1;padding:13px;border-radius:14px;border:none;background:linear-gradient(135deg,#2a84c9,#1a5fa0);color:#fff;font-size:14px;font-weight:900;cursor:pointer;transition:.2s;display:flex;align-items:center;justify-content:center;gap:8px;}
.btn-submit:hover{transform:translateY(-1px);}
.btn-cancel{padding:13px 22px;border-radius:14px;background:#f0f5fb;color:#21425f;font-size:13px;font-weight:900;text-decoration:none;display:flex;align-items:center;justify-content:center;}
...
<div class="page-header">
    ...
    <div>
        <h1>✏️ تعديل مشرف</h1>
        <p>تعديل بيانات وصلاحيات: <?php echo htmlspecialchars($sup['full_name']) ?></p>
    </div>
</div>
        ...
        <div class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="field">
                    ...
                    <div class="field-wrap"><span class="field-icon">👤</span>
                        <input type="text" name="full_name" id="fullName" value="<?php echo htmlspecialchars($_POST['full_name'] ?? $sup['full_name']) ?>" required>
                    </div>
                </div>
                <div class="field">
                    <label>اسم المستخدم</label>
                    <div class="field-wrap"><span class="field-icon">@</span>
                        <input type="text" name="username" value="<?php echo htmlspecialchars($_POST['username'] ?? $sup['username']) ?>" required>
                    </div>
                ...
                <div class="field">
                    ...
                    <div class="field-wrap"><span class="field-icon">🔑</span>
                        <input type="password" name="password" id="passField" placeholder="اتركها فارغة للإبقاء على القديمة">
                    </div>
                    <div class="field-hint" id="passHint">اتركها فارغة إذا لم تريد تغييرها</div>
                </div>
            </div>
        </div>
            ...
            <table class="perm-table">
                ...
                foreach ($sections as $key => $s):
                    ...
                    if ($group !== $lastGroup && isset($groupLabels[$group]) && $groupLabels[$group] !== ''):
                        $lastGroup = $group;
                ?>
                <tr class="group-sep"><td colspan="5"><?php echo $groupLabels[$group] ?></td></tr>
                <?php endif;
                    $p  = $curPerms[$key] ?? ['can_view'=>0,'can_add'=>0,'can_edit'=>0,'can_delete'=>0];

                    if ($isMaster) {
                        if (isset($_POST['full_name'])) {
                            $en = isset($_POST["perm_{$key}"]) ? 1 : 0;
                        } else {
                            $en = ($p['can_view'] || $p['can_add'] || $p['can_edit'] || $p['can_delete']) ? 1 : 0;
                        }
                    } elseif ($group === 'gc_sub') {
                        $ce = isset($_POST['full_name']) ? (isset($_POST["perm_{$key}_edit"]) ? 1 : 0) : intval($p['can_edit']);
                    } elseif ($key === 'chat') {
                        $cv = isset($_POST['full_name']) ? (isset($_POST['perm_chat_view']) ? 1 : 0) : intval($p['can_view']);
                        $ca = isset($_POST['full_name']) ? (isset($_POST['perm_chat_add'])  ? 1 : 0) : intval($p['can_add']);
                    } elseif ($key === 'reports') {
                        $cv = isset($_POST['full_name']) ? (isset($_POST['perm_reports_view']) ? 1 : 0) : intval($p['can_view']);
                    } else {
                        $cv = isset($_POST['full_name']) ? (isset($_POST["perm_{$key}_view"])   ? 1 : 0) : intval($p['can_view']);
                        $ca = isset($_POST['full_name']) ? (isset($_POST["perm_{$key}_add"])    ? 1 : 0) : intval($p['can_add']);
                        $ce = isset($_POST['full_name']) ? (isset($_POST["perm_{$key}_edit"])   ? 1 : 0) : intval($p['can_edit']);
                        $cd = isset($_POST['full_name']) ? (isset($_POST["perm_{$key}_delete"]) ? 1 : 0) : intval($p['can_delete']);
                    }
                    $masterCls = ($group === 'gc_master') ? 'master-gc' : (($isMaster) ? 'master-sub' : '');
                ?>
                <?php if ($isMaster): ?>
                <tr class="row-master">
                    <td class="indent-<?php echo $indent ?>">
                        <span class="master-badge <?php echo $masterCls ?>">ماستر</span>
                ...
                <tr>
                    ...
                    <td>
                        <div class="toggle-wrap">
                            <label class="toggle">
                                <input type="checkbox" name="perm_chat_view"
                                       class="perm-cb perm-view" data-key="chat"
                                       <?php echo $cv ? 'checked' : '' ?>>
                                <span class="toggle-slider"></span>
                            </label>
                        </div>
                    ...
                    <td>
                        <div class="toggle-wrap">
                            <label class="toggle">
                                <input type="checkbox" name="perm_chat_add"
                                       class="perm-cb perm-add"
                                       <?php echo $ca ? 'checked' : '' ?>
                                       <?php echo !$cv ? 'disabled' : '' ?>>
                                <span class="toggle-slider"></span>
                            </label>
                        </div>
                ...
                <tr>
                    ...
                    <td>
                        <div class="toggle-wrap">
                            <label class="toggle">
                                <input type="checkbox" name="perm_reports_view"
                                       class="perm-cb perm-view" data-key="reports"
                                       <?php echo $cv ? 'checked' : '' ?>>
                                <span class="toggle-slider"></span>
                            </label>
                        </div>
...
passInput.addEventListener('input', function() {
    ...
    if (!l)       { passHint.textContent='اتركها فارغة إذا لم تريد تغييرها'; passHint.style.color='#7a90a3'; }
